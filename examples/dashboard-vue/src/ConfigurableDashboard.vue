...
<template>
  <div class="configurable-dashboard">
    ...
    <main class="dashboard-main">
      ...
      <section v-if="state.filterDefinitions.length" class="filter-bar" aria-label="Dashboard filters">
        <template v-for="definition in state.filterDefinitions" :key="definition.id">
          <label v-if="isDateFilter(definition)" class="date-filter"><span>{{ definition.id }}</span><input type="date" :value="dateValue(definition.id, 0)" @change="(event) => setDateValue(definition, 0, event)" /><input type="date" :value="dateValue(definition.id, 1)" @change="(event) => setDateValue(definition, 1, event)" /></label>
          <div v-else-if="isRangeFilter(definition)" class="filter-item"><span>{{ definition.id }}</span><input v-for="index in [0, 1]" :key="index" type="number" :data-index="index" :min="definition.min" :max="definition.max" :value="rangeValue(definition)[index]" @change="(event) => setRangeValue(definition, event)" /></div>
          <label v-else class="filter-item"><span>{{ definition.id }}</span><select :value="filterValue(definition.id)" @change="(event) => setSelectValue(definition, event)"><option v-if="definition.allValue !== undefined" :value="definition.allValue">全部</option><option v-for="option in optionsFor(definition)" :key="String(option.value)" :value="option.value">{{ option.label }}</option></select></label>
        </template>
    ...
    <a-drawer :open="Boolean(activePanel)" :title="activePanel?.title" width="min(520px, 94vw)" @close="clearSelection"><template v-if="activePanel"><a-descriptions :column="1" size="small" bordered><a-descriptions-item label="Panel ID">{{ activePanel.id }}</a-descriptions-item><a-descriptions-item label="指标">{{ activePanel.query.metrics.map((metric) => metric.metricId).join(', ') }}</a-descriptions-item><a-descriptions-item label="维度">{{ activePanel.query.dimensions.map((dimension) => dimension.dimensionId).join(', ') || '-' }}</a-descriptions-item></a-descriptions><a-divider /><pre class="json-block">{{ stringify(runtime.queryForPanel(activePanel)) }}</pre></template></a-drawer>
