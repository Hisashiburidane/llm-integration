...
import { Alert, Button, Card, Col, Drawer, Layout, Menu, Modal, Row, Space, Statistic, Switch, Tag, Tooltip, Typography } from "antd";
import ReactECharts from "echarts-for-react";
import { useEffect, useMemo, useRef, useState } from "react";
import { createTopicCharts, timeRangeLabel, topics, type ChartMeta, type TimeRange, type TopicKey } from "./data";
import { buildUiContext, extractToolCalls, recommendPrompts, uiTool, type UiActionCall } from "./llmRuntime";
...
export default function App() {
  ...
  function executeCalls(calls: UiActionCall[]) {
    calls.forEach(executeAction);
    addMessage("ai", `LLM 已执行 ${calls.length} 个动作：${calls.map((call) => call.action).join("、")}。`);
  }

  async function callLlm(text: string) {
    if (__LLM_CONFIG_ERROR__) { addMessage("ai", `${__LLM_CONFIG_ERROR__}，请检查 examples/react-dashboard/.env。`); return; }
    setLoading(true);
    try {
  ...
  function submitAssistant(text: string) {
